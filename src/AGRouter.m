...
- (NSDictionary *)route:(NSString *)method path:(NSString *)path body:(NSString *)body {
    ...
    // === Web 控制台仪表盘 (/) ===
    if ([path isEqualToString:@"/"] || [path isEqualToString:@"/dashboard"]) {
        return [self respondHTML:[AGRouter dashboardHTML]];
    }
...
+ (NSString *)apiDocsHTML {
    ...
    "# 截图\ncurl -X POST http://设备IP:8888/api/screen/screenshot\n\n"
    "# 设置亮度\ncurl -X POST http://设备IP:8888/api/screen/brightness -d '{\"value\":80}'\n\n"
    "# 点击坐标\ncurl -X POST http://设备IP:8888/api/touch/tap -d '{\"x\":300,\"y\":500}'\n\n"
    "# 输入文字\ncurl -X POST http://设备IP:8888/api/input/text -d '{\"text\":\"Hello World\"}'\n\n"
    "# 启动 App\ncurl -X POST http://设备IP:8888/api/apps/launch -d '{\"bundleId\":\"com.apple.Preferences\"}'\n\n"
    "# 执行命令\ncurl -X POST http://设备IP:8888/api/shell/exec -d '{\"command\":\"uptime\"}'\n\n"
    "# MCP 协议\ncurl -X POST http://设备IP:8888/mcp -d '{\"jsonrpc\":\"2.0\",\"method\":\"tools/list\",\"id\":1}'\n"
    "</pre>"
...
+ (NSString *)dashboardHTML {
    ...
    "padding:28px 20px 20px;text-align:center;border-bottom:1px solid rgba(255,255,255,.06)}"
    ".header h1{font-size:24px;font-weight:700;"
    "background:linear-gradient(90deg,#64d2ff,#0a84ff);-webkit-background-clip:text;"
    "-webkit-text-fill-color:transparent;margin-bottom:4px}"
    ".header .sub{font-size:12px;color:#636366}"
    ".container{max-width:800px;margin:0 auto;padding:16px}"
    ".status-bar{display:flex;align-items:center;justify-content:space-between;"
    "background:#14141c;border-radius:14px;padding:16px 20px;margin-bottom:12px;"
    "border:1px solid rgba(255,255,255,.06)}"
    ".status-left{display:flex;align-items:center;gap:10px}"
    ".status-dot{width:12px;height:12px;border-radius:50%;background:#555;"
    "animation:pulse 2s infinite}"
    ".status-dot.active{background:#30d158;box-shadow:0 0 12px rgba(48,209,88,.4)}"
    ".status-dot.inactive{background:#ff453a;box-shadow:0 0 12px rgba(255,69,58,.4)}"
    "@keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}}"
    ".status-text{font-size:16px;font-weight:600}"
    ".status-text.active{color:#30d158}.status-text.inactive{color:#ff453a}"
    ".uptime{font-size:12px;color:#636366}"
    ".grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px}"
    ".metric{background:#14141c;border:1px solid rgba(255,255,255,.06);"
    "border-radius:12px;padding:14px 16px;transition:border-color .15s}"
    ".metric .label{font-size:11px;color:#636366;text-transform:uppercase;"
    "letter-spacing:.5px;margin-bottom:6px}"
    ".metric .value{font-size:20px;font-weight:700;color:#fff}"
    ".metric .value.battery{color:#ffd60a}"
    ".metric .value.memory{color:#64d2ff}"
    ".metric .value.storage{color:#bf5af2}"
    ".metric .sub-value{font-size:11px;color:#636366;margin-top:2px}"
    ...
    "text-decoration:none;cursor:pointer;border:none;transition:all .15s;text-align:center}"
    ".btn-primary{background:#0a84ff;color:#fff}"
    ".btn-outline{background:transparent;color:#0a84ff;border:1px solid rgba(10,132,255,.3)}"
    ".section{margin-bottom:12px}"
    ".section-title{font-size:11px;color:#636366;text-transform:uppercase;"
    "letter-spacing:.5px;margin-bottom:8px;padding-left:4px}"
    ".ep-list{display:flex;flex-direction:column;gap:3px}"
    ".ep{display:flex;align-items:center;gap:8px;padding:8px 12px;background:#14141c;"
    "border-radius:8px;font-size:13px;border:1px solid rgba(255,255,255,.04);"
    "text-decoration:none;color:#e0e0e0;transition:background .15s;cursor:pointer}"
    ".ep:hover{background:#1c1c28}"
    ".badge{display:inline-flex;padding:2px 7px;border-radius:4px;font-size:10px;font-weight:700;min-width:36px;justify-content:center}"
    ".badge.get{background:rgba(97,175,254,.15);color:#61affe}"
    ".badge.post{background:rgba(73,204,144,.15);color:#49cc90}"
    ".ep .apath{font-family:SF Mono,Menlo,monospace;font-size:12px;flex:1;"
    "overflow:hidden;text-overflow:ellipsis;white-space:nowrap}"
    ".ep .adsc{font-size:11px;color:#636366;flex-shrink:0}"
    ".refresh{display:flex;align-items:center;justify-content:space-between;"
    "padding:8px 4px;font-size:12px;color:#636366}"
    ".refresh span{cursor:pointer;color:#0a84ff}"
    ".footer{text-align:center;padding:20px;font-size:11px;color:#3a3a3c}"
    "@media(min-width:600px){.grid{grid-template-columns:repeat(4,1fr)}}"
    "</style></head><body>"
    "<div class=header>"
    "<h1>AutoGo 控制模块</h1>"
    "<div class=sub>iOS 设备远程控制 · 60+ API · MCP 协议</div></div>"
    "<div class=container>"
    "<div class=status-bar id=sbar>"
    "<div class=status-left>"
    "<div class=status-dot id=sdot></div>"
    "<span class=status-text id=stxt>检测中...</span></div>"
    "<span class=uptime id=sut></span></div>"
    "<div class=grid>"
    "<div class=metric>"
    "<div class=label>设备名称</div>"
    "<div class=value id=mDev>--</div>"
    "<div class=sub-value id=mOS>--</div></div>"
    "<div class=metric>"
    "<div class=label>电池电量</div>"
    "<div class='value battery' id=mBat>--</div>"
    "<div class=sub-value id=mCharge></div></div>"
    "<div class=metric>"
    "<div class=label>内存</div>"
    "<div class='value memory' id=mMem>--</div>"
    "<div class=sub-value id=mMemDet></div></div>"
    "<div class=metric>"
    "<div class=label>存储空间</div>"
    "<div class='value storage' id=mStor>--</div>"
    "<div class=sub-value id=mStorDet></div></div></div>"
    "<div class=actions>"
    "<a class='btn btn-primary' href=/api/docs>📖 API 文档</a>"
    "<span class='btn btn-outline' onclick=refresh() style=cursor:pointer>🔄 刷新状态</span></div>"
    "<div class=section>"
    "<div class=section-title>📱 常用接口</div>"
    "<div class=ep-list>"
    "<a class=ep href=/api/device/info>"
    "<span class='badge get'>GET</span>"
    "<span class=apath>/api/device/info</span>"
    "<span class=adsc>设备信息</span></a>"
    "<a class=ep href=/api/screen/screenshot>"
    "<span class='badge post'>POST</span>"
    "<span class=apath>/api/screen/screenshot</span>"
    "<span class=adsc>截图</span></a>"
    "<a class=ep href=/api/apps/list>"
    "<span class='badge get'>GET</span>"
    "<span class=apath>/api/apps/list</span>"
    "<span class=adsc>App 列表</span></a>"
    "<a class=ep href=/api/system/processes>"
    "<span class='badge get'>GET</span>"
    "<span class=apath>/api/system/processes</span>"
    "<span class=adsc>进程列表</span></a>"
    "<a class=ep href=/api/wifi/info>"
    "<span class='badge get'>GET</span>"
    "<span class=apath>/api/wifi/info</span>"
    "<span class=adsc>WiFi 信息</span></a>"
    "<a class=ep href=/api/system/memory>"
    "<span class='badge get'>GET</span>"
    "<span class=apath>/api/system/memory</span>"
    "<span class=adsc>内存详情</span></a></div></div>"
    "<div class=refresh>"
    "<span>每 5 秒自动刷新</span>"
    "<span onclick=refresh()>立即刷新</span></div></div>"
    "<div class=footer>AutoGo Daemon v1.0.0 · Rootless · 端口 8888</div>"
    "<script>"
    "var st=Date.now(),timer=null;"
    "function fmtUptime(ms){var s=Math.floor(ms/1000),m=Math.floor(s/60),h=Math.floor(m/60);"
    "if(h>0)return h+'小时 '+m%60+'分';if(m>0)return m+'分 '+s%60+'秒';return s+'秒'}"
    "function setActive(d){"
    "var dot=document.getElementById('sdot'),txt=document.getElementById('stxt');"
    "dot.className='status-dot active';txt.className='status-text active';"
    "txt.textContent='服务已激活';"
    "document.getElementById('sut').textContent='运行时间: '+fmtUptime(Date.now()-st);"
    "if(d&&d.device){"
    "document.getElementById('mDev').textContent=d.device.name||'--';"
    "document.getElementById('mOS').textContent='iOS '+(d.device.osVersion||'--');"
    "var bl=d.device.batteryLevel;"
    "document.getElementById('mBat').textContent=bl!=null?Math.round(bl*100)+'%':'--';"
    "document.getElementById('mCharge').textContent=d.device.isCharging?'⚡充电中':'';"
    "document.getElementById('mMem').textContent=(d.device.memoryTotal||'--')+' GB';"
    "document.getElementById('mMemDet').textContent='可用 '+(d.device.memoryFree||'--')+' GB';"
    "document.getElementById('mStor').textContent=(d.device.storageTotal||'--')+' GB';"
    "document.getElementById('mStorDet').textContent='可用 '+(d.device.storageFree||'--')+' GB';"
    "}}"
    "function setInactive(){"
    "document.getElementById('sdot').className='status-dot inactive';"
    "document.getElementById('stxt').className='status-text inactive';"
    "document.getElementById('stxt').textContent='服务未激活'}"
    "function refresh(){"
    "fetch('/api/device/info').then(function(r){return r.json()}).then(function(d){"
    "if(d&&d.data)setActive(d);else setInactive()}).catch(function(){setInactive()})}"
